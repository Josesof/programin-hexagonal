package com.company.game.infraestructure.controlles;
...
public class TaskController {
  ...
  public ResponseEntity<Task> createTask(@RequestBody Task task) {
    Task createTask = taskService.createTasks(task);
    return new ResponseEntity<>(createTask, HttpStatus.CREATED);
  }

  @GetMapping("/{taskId}")
  public  ResponseEntity<Task> getTaskById(@PathVariable Long taskId){
    return taskService.getTasks(taskId).map(task -> new ResponseEntity<>(task, HttpStatus.OK))
  ...
  public  ResponseEntity<Task> update(@PathVariable Long taskId, @RequestBody Task task){
    return taskService.updateTasks(taskId, task).map(task1 -> new ResponseEntity<>(task, HttpStatus.OK))
      .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
